...
### 2.10 내부 가용 리소스 데이터 (승격 전 적합성 판단용)

승격 판단은 "시장이 매력적인가"만이 아니라 **"우리가 지금 대응할 수 있는가"** 를 함께 봐야 한다. 이를 위한 내부 리소스 스키마 (`data/sample/internal_resources.json`):

| 데이터 | 필드 | 실서비스 연계 소스(안) |
|---|---|---|
| 영업 담당 | 이름, 담당 사업부, 채널, 세그먼트, 전문 분야 | HR 조직도 + Salesforce Role |
| 담당별 진행중 로드 | 진행중 건수·금액 (저장 안 함 — 사업기회에서 **실시간 집계**) | Salesforce Opportunity |
| 기술지원(SE) | 사업부별 인원, 가동률 %, 동시 PoC 가능 건수 | 리소스 관리 시트/PSA |
| 데모 장비 | 제품군·모델별 보유/대여중 대수 | 자산관리 시스템 |
| 총판 파트너 | 취급 사업부·제품군, 커버 지역 | 파트너 마스터 |

**유사 사업기회 검색 — 임베딩 기반**: 과거 사업기회를 "사업기회명+제품군+Pain Point+세그먼트" 문장으로 로컬 임베딩 모델(`intfloat/multilingual-e5-small`, sentence-transformers)에 통과시켜 벡터 캐시하고, 이벤트(기관명+요약+추정 제품군)와 코사인 유사도 Top-3를 반환한다. 모델 미설치 환경에서는 한국어 음절 2-gram TF-IDF 코사인으로 자동 폴백(응답 `method` 필드로 구분, UI에 방식 표기). 구현: `backend/similarity.py`.

### 2.11 기술 트렌드 팔로업 데이터 (원시 시그널 → 트렌드 클러스터)

Epson 보유 기술영역(사업부별)의 산업·기술 방향을 추적하기 위한 2계층 구조. 특허·뉴스 1건이 아니라 **유사 시그널의 누적**을 트렌드로 본다 (센싱 이벤트→승격 사업기회와 같은 발상).

**원시 시그널** (`data/sample/tech_signals.json`) — 소스 4종, 크롤링 배제:

| sourceType | 소스 | detail.type | 상세속성 |
|---|---|---|---|
| PATENT | KIPRIS Open API | patent | 출원번호, IPC 분류, 출원인 |
| RND | NTIS Open API | rnd_program | 과제번호, 주관기관, 예산 |
| NEWS | 빅카인즈 Open API (조건부 — DATA_SOURCE_FEASIBILITY 7절 판정) | news | 매체, 기사 분류 |
| COMPETITOR | 경쟁사 공식 IR·보도자료 | competitor_release | 발표 유형, 제품 라인 |

공통 필드는 센싱 이벤트와 동일 컨벤션(eventDate/source/sourceUrl/reliability/detail 판별자). `trendId`로 클러스터에 연결되며, 아직 유사 시그널이 없는 건은 `trendId: null`(미배정 — 신규 트렌드 후보).

**트렌드 클러스터** (`data/sample/tech_trends.json`): trendId, title, businessUnit[], techCategory, summary(판단 서술), signalIds[](시그널 수는 배열 길이로 계산), firstSeen/lastSeenDate, momentum(증가/유지/감소 — 예시는 수동 평가, 실서비스는 시그널 발생 빈도로 자동 산출), relevanceScore(1~5 — Epson 기술영역 연관성·사업 영향도 수동 평가, 기준 룰 확정 필요), status, tags[].

**트렌드 상태 — 사람의 처리 단계** (`PATCH /api/tech-trends/{id}`, 센싱 이벤트 상태와 같은 원리):

```
신규(자동: 클러스터링 직후, 미검토) →[추적 시작]→ 추적중 →[보고 처리]→ 보고완료
어느 상태에서든 [중단] → 중단 (모니터링 제외 판단, 하단 별도 목록) →[재개]→ 추적중
```

"신규"는 **아직 아무도 검토하지 않은 인박스**, "추적중"은 담당자가 팔로업하기로 결정한 건이다 — 데이터 축적 정도(그건 momentum·시그널 수가 표현)가 아니라 사람의 처리 여부로 구분한다.

**미배정 시그널 vs 시그널 1건 트렌드**: 차이는 "하나의 기술 흐름"이라는 판단 여부다. 미배정 시그널은 아직 아무 판단이 없는 낱개 수집 데이터, 1건 트렌드는 트렌드 등록 판단(제목·사업부·관련도·태그 부여)이 끝난 초기 포착이다. 승격 기준(안): ① 유사 시그널 2건 이상 누적 시 자동으로 신규 트렌드 후보 생성 ② 국책 R&D 과제·대형 투자 공시처럼 단일 시그널이라도 강도가 높으면 담당자 수동 등록 — 기준 확정은 Open Questions #14.

**트렌드 ↔ 파이프라인 연결** (`GET /api/tech-trends/{id}/related`): 사업부가 겹치는 사업기회·센싱 이벤트를 2.10의 유사도 엔진으로 매칭(방식별 최소 유사도 `RELATED_FLOOR` 적용). 모멘텀 "증가"인데 연결 파이프라인이 0건이면 "대응 기회 발굴 필요" 경고를 표시한다 — 트렌드 팔로업이 '읽고 끝'이 되지 않게 하는 핵심 장치. 실서비스에서는 자동 매칭 위에 담당자 수동 확정 링크를 병행한다.
...
| 2-G | 대응 적합성 판단 | 이벤트 상세 패널 내 섹션 | 승격 전 4개 축 점수 + 근거 + 유사사례 카드 + 추천 담당·파트너 + 주의사항 (아래 산식) |

- 공통 필터: 기간 · 구분(B2B/B2G) · 트리거유형 · 사업부(추정) · 상태 · 스코어
- **승격 액션 동작**: 이벤트 → 사업기회 생성 폼(이벤트 내용 자동 프리필: 계정, 제품군, 잠재규모→예상금액, LeadSource=MI센싱) → 저장 시 탭1 파이프라인 단계1로 진입 + 이벤트 상태 "승격" 처리. 폼 상단에 적합성 요약 표시, 담당 미배정 건은 추천 담당(로드 최소)을 기본값으로 프리필

**2-G 대응 적합성 산식** (`GET /api/fit-assessment/{eventId}`):

```
적합성 = 0.35×유사사례 실적 + 0.25×영업 가용성 + 0.25×기술지원·데모 여력 + 0.15×채널·파트너 커버리지
권고: ≥70 승격 권장 / 40~69 조건부 승격 / <40 보류 권장
```

| 축 | 산정 기준 |
|---|---|
| 유사사례 실적 | 임베딩 유사도 Top-3 최고치 ×100, 수주 사례 +12/건(최대 2건), 실주 사례 −15/건 |
| 영업 가용성 | 95 − 14×(배정/추천 담당의 진행중 건수). 5건 이상이면 주의사항 |
| 기술지원·데모 여력 | (100 − 사업부 SE 가동률) + 데모 장비 잔여 있으면 +10 / 전량 대여중 −20, B2G 입찰 마감 D-14 이내 −10 |
| 채널·파트너 커버리지 | 기존 계정 85(총판+커버 파트너 90) / 신규+커버 파트너 65 / 커버 파트너 없음 50 |

> 판단은 항상 **근거 문자열과 함께** 반환한다(블랙박스 점수 금지). 동일 사업부의 경쟁사 낙찰·스펙락인 이력은 주의사항으로 병기.

### 3.3 탭3 — 기술 트렌드 (Epson 보유 기술영역 팔로업)

| # | 컴포넌트 | 형태 | 정의 |
|---|---|---|---|
| 3-A | 트렌드 요약 카드 ×4 | 숫자 카드 | ① 추적중 트렌드 ② 신규 포착 ③ 모멘텀 증가 ④ **미배정 시그널**(신규 트렌드 후보, 경고 색) |
| 3-B | 트렌드 카드 리스트 | 카드 (관련도순) | 제목 + 사업부 + 상태 태그 + 모멘텀 + 관련도 도트 + 시그널 수·소스 구성 + **월별 시그널 축적 미니 타임라인** |
| 3-C | 트렌드 상세 드로어 | 우측 슬라이드 | 개요(dl) + 판단 서술 + 근거 시그널 시간순 카드(소스별 상세속성·원문 링크) + **관련 파이프라인**(아래) |
| 3-D | 관련 파이프라인 | 상세 드로어 내 섹션 | 유사도 매칭된 사업기회(단계·금액·담당)와 센싱 이벤트(상태·승격 링크). 모멘텀 증가 + 파이프라인 0건이면 "대응 기회 발굴 필요" 경고 |
| 3-E | 미배정 시그널 | 리스트 | trendId 없는 시그널 — 유사 시그널 축적 시 신규 트렌드 승격 검토 |
| 3-F | 상태 처리 액션 | 카드·드로어 버튼 | 신규→[추적 시작]/[중단], 추적중→[보고 처리]/[중단], 보고완료→[중단], 중단→[재개]. 2.11 상태 전이 저장 |
| 3-G | 중단된 트렌드 | 하단 별도 섹션 (흐림) | 모니터링 제외 건만 모아 표시, [재개] 가능 — 본 목록·KPI 집계에서 제외 |

- 필터: 사업부(공통 헤더) · 상태 · 모멘텀 · 정렬(관련도/최근 시그널순)
- 기간 필터는 트렌드에 적용하지 않는다(클러스터는 수개월에 걸쳐 축적되므로 연/월 필터로 자르면 왜곡됨)
...
| 13 | 내부 리소스 데이터 소스 | SE 가동률·데모 장비 재고·파트너 커버리지의 실제 관리 시스템과 갱신 주기 (현재는 예시 JSON) | 실무·IT |
| 14 | 트렌드 클러스터링·평가 기준 | 시그널→트렌드 묶음 기준(수동/자동), momentum 자동 산출식, relevanceScore 평가 룰 | 실무·데이터 담당 |
| 15 | 빅카인즈 상업 이용 약정 | 뉴스 시그널 수집용 빅카인즈 API 유료 약정 범위·비용 (DATA_SOURCE_FEASIBILITY 7절 참조) | 법무·IT |
...
| `data/sample/internal_resources.json` | 내부 가용 리소스 (영업 담당·SE·데모 장비·파트너 커버리지) — 적합성 판단용 |
| `data/sample/tech_signals.json` | 기술 트렌드 원시 시그널 13건 (특허·R&D과제·뉴스·경쟁사 발표) |
| `data/sample/tech_trends.json` | 기술 트렌드 클러스터 5건 (시그널 묶음 + 모멘텀·관련도·상태) |
