...
**트렌드 클러스터** (`data/sample/tech_trends.json`): trendId, title, businessUnit[], techCategory, summary(판단 서술), signalIds[](시그널 수는 배열 길이로 계산), firstSeen/lastSeenDate, momentum(증가/유지/감소 — 예시는 수동 평가, 실서비스는 시그널 발생 빈도로 자동 산출), relevanceScore(1~5 — Epson 기술영역 연관성·사업 영향도 수동 평가, 기준 룰 확정 필요), status(신규→추적중→보고완료), tags[].
