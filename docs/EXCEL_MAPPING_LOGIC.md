...
flowchart TD
    ...
    D --> M["1단계: 컬럼 매핑 화면<br/>원본 컬럼(샘플 표시) → 셀포 대응 필드<br/>자동 제안 + 사용자 드롭다운 조정"]
    M -->|"이 매핑으로 미리보기"| F[행별 해석<br/>§3 값 해석 · §4 ID 부여]
...
업로드 직후 **컬럼 매핑 화면**이 먼저 뜬다: 감지된 원본 컬럼마다 샘플 값과 함께 자동 제안된 대상 필드가
드롭다운으로 표시되고(대상 필드에는 Salesforce 대응 필드명 병기 — 예: 단계 → StageName), 사용자가 직접
바꾸거나 "매핑 안 함"으로 제외한 뒤 확정한다. 자동 제안은 아래 유사어 사전 기준이다.
(제목행은 건너뛰고 1~5행에서 헤더 행을 탐색)
