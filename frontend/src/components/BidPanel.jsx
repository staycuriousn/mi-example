// B2G 입찰 타임라인(2-E) + 경쟁사 낙찰 동향(2-F)
const dayOf = s => Math.floor(new Date(`${s}T00:00:00Z`).getTime() / 86400000)

export function BidTimeline({ bids, today }) {
  if (bids.length === 0)
    return (
      <div className="statebox">
        <strong>기간 내 B2G 입찰 이벤트가 없습니다</strong>
        필터(기간·사업부)를 조정해 보세요.
      </div>
    )

  const t = dayOf(today)
  const items = bids.map(e => {
    const d = e.detail
    const start = dayOf(d.noticeDate)
    const end = dayOf(d.bidDeadline)
    return { e, d, start, end, award: d.awardDate ? dayOf(d.awardDate) : null }
  })
  ...
  const fmtRange = i => `${i.d.noticeDate.slice(5)}~${i.d.bidDeadline.slice(5)}`

  return (
    <div className="panel">
      <div className="tl">
        {items.map(i => {
          const closed = i.end < t
          const urgent = !closed && i.end - t <= 7
          return (
            <div key={i.e.eventId} style={{ display: 'contents' }}>
              <span className="tlname" title={i.e.summary}>
                {i.d.orderingOrg} · {i.d.itemCategory}
              </span>
              <div className="tltrack">
                <div
                  className={`tlbar ${urgent ? 'urgent' : ''} ${closed ? 'closed' : ''}`}
                  style={{ left: pct(i.start), width: widthPct(i.start, i.end) }}
                  title={`공고 ${i.d.noticeDate} → 마감 ${i.d.bidDeadline}${urgent ? ` (D-${i.end - t})` : ''}`}
                >
                  {fmtRange(i)}{urgent ? ` · D-${i.end - t}` : ''}
                </div>
                {i.award && <div className="tlaward" style={{ left: pct(i.award) }} title={`낙찰 ${i.d.awardDate}`} />}
      ...
      <div className="legend" style={{ marginTop: 8 }}>
        <span className="li"><span className="swatch" style={{ background: 'var(--ink-300)' }} /> 진행중 공고</span>
        ...
        <span className="li"><span className="swatch" style={{ background: 'var(--ink)', width: 3 }} /> 기준일</span>
      </div>
    </div>
  )
}
